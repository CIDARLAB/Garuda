...
public class TestStatistic {
    ...
    */
    ...
    public static double chisq (double[] observed, double[] expected) {
        
        double cv = 0.0;
        double xSqr;
        
        if (observed.length!=expected.length) {
            System.out.println ("***Error! the length of the arrays do not match.");
        }
        ...
        for (int i=0; i<observed.length; i++)
        {
            ...
            cv += ((xSqr * xSqr) / expected[i]);
        }
        
        return pchisq (cv, dof);
    }
}
